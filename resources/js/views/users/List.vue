  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="Actions" width="350">
        <template slot-scope="scope">
          <router-link
            v-if="!scope.row.roles.includes('root')"
          ...
          <el-button
            v-if="!scope.row.roles.includes('root')"
            v-permission="['system.admin.permission']"
            type="warning"
            size="small"
            icon="el-icon-edit"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      roles: ['root', '管理员', 'seo', '演示'],
      nonAdminRoles: ['管理员', 'seo', '演示'],
      newUser: {},
  ...
  computed: {
    normalizedOtherPermissions() {
      ...
      const rolePermissions = {
        id: -1,
        display_name: 'Inherited from role',
        disabled: true,
      ...
      const userPermissions = {
        id: 0,
        display_name: 'Extra permissions',
        children: tmp,
  ...
  created() {
    this.resetNewUser();
    this.getList();
    if (checkPermission(['system.admin.permission'])) {
      this.getPermissions();
    }
  },
  methods: {
    ...
    normalizePermission(permission) {
      const disabled =
        permission.disabled || permission.name === 'system.admin.permission';
      return {
        id: permission.id,
        name: permission.name,
        display_name: permission.display_name,
